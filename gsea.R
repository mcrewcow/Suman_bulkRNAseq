...
for(i in 1:length(folders)) {                              # Head of for-loop
  assign(paste0("csv", i),                                   # Read and store data frames
         read.csv(paste(folders[i], "GSEA_result.csv", sep="/"))[c("ID","NES","pvalue")])
}
...
gsea_full <- Reduce(function(...) merge(..., by='ID', all=TRUE), csv_list)

#replace NA with 0
gsea_full[is.na(gsea_full)] <- 1

#write file
write.csv(gsea_full,file=paste("full_gsea_results.csv"), row.names=FALSE)
